...
output "ansible_inventory" {
  value = <<EOT
[frontend]
frontend ansible_host=${aws_instance.frontend.private_ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/${var.key_name}.pem private_ip=${aws_instance.frontend.private_ip}

[backend]
backend ansible_host=${aws_instance.backend.private_ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/${var.key_name}.pem private_ip=${aws_instance.backend.private_ip}

[worker]
worker ansible_host=${aws_instance.worker.private_ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/${var.key_name}.pem private_ip=${aws_instance.worker.private_ip}
